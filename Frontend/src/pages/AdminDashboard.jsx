...
const AdminOrderTable = () => {
    ...
    useEffect(() => {
        axios.get('https://food-delivery-app-backend-oihz.onrender.com/api/v1/order/get')
            ...
            for (const order of orders) {
                for (const itemId of Object.keys(order.allItemsWithQuant)) {
                    if (!categoryNames[itemId]) {
                        try {
                            const response = await axios.get(`https://food-delivery-app-backend-oihz.onrender.com/api/v1/users/food/${itemId}`);
    ...
    const handleStatusChange = async (orderId, newStatus) => {
        try {
            const response = await axios.put(`https://food-delivery-app-backend-oihz.onrender.com/api/v1/order/orderstatus/${orderId}`, { status: newStatus });
